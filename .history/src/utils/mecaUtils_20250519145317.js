...
export function piocherCarte( sourceType, targetType, draggedCard, deck, setDeck, boardPlayer, setBoardPlayer, setFusionAnim ){
    ...
    if (sourceType === "board" && targetType === "header" && deck.length <7){
        ...
        if(draggedCard.piocherCarteSpeApresVente){
            let clone = []
            const carteSpe = cards[draggedCard.carteSpe]
            let cloneAfusionnerBoard = boardPlayer.filter((c)=> c.nom === carteSpe.nom)
            let cloneAfusionnerDeck = deck.filter((c)=> c.nom === carteSpe.nom)
            console.log(cloneAfusionnerBoard, cloneAfusionnerDeck)
            ...
            if(cloneAfusionnerBoard.length + cloneAfusionnerDeck.length === 3){
                let newBoard = boardPlayer.filter((c)=> c !== draggedCard && c.nom !== clone.nom)
