...
export async function verifElemOnDrag(draggedCard, boardPlayer, setBoardPlayer, shopCards, bonusHpElem, bonusAtkElem, setBonusHpElem, sourceType, targetType){
                //ELEMENTAIRES
            //si pose Board
...
export function appliquerEffetDeCouple(carteAjoutee, cartesBoard) {
...
if (sourceType === "board" 
    ...
    draggedCard.bivalenceSell(futurBoard, draggedCard)
  }else{
...
if (sourceType === "deck" 
  ...
  if(draggedCard.bivalence){
    draggedCard.bivalence(futurBoard, draggedCard)
    // Faire réagir les autres cartes à l’arrivée de celle-ci
    futurBoard.forEach(c => {
      if (c !== draggedCard && c.bivalence) {
        c.bivalence(futurBoard, c); // chaque carte réévalue le board et s'applique à la nouvelle
